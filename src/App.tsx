...
function App() {
  ...
  const simulateProgress = () => {
    ...
    const interval = setInterval(() => {
      ...
      const stepIndex = Math.min(Math.floor(currentProgress / 5.5), steps.length - 1);
  ...
  const handleAnalyze = async (url: string) => {
    ...
    try {
      ...
      if (err.code === 'ECONNREFUSED' || err.message?.includes('Network Error')) {
        errorMessage = 'Backend server is not running. Please start the Python backend server on port 8000.';
                  ...
                  {error.includes('Backend server is not running') && (
                    <div className="mt-3 p-3 bg-red-100 rounded">
                      ...
                      <ol className="list-decimal list-inside mt-2 space-y-1">
                        ...
                        <li>Run: <code className="bg-red-200 px-1 rounded">pip install -r requirements.txt</code></li>
                        <li>Run: <code className="bg-red-200 px-1 rounded">python main.py</code></li>
                  ...
                  {error.includes('Analysis timeout') && (
                    <div className="mt-3 p-3 bg-yellow-100 rounded">
                      ...
                      <ul className="list-disc list-inside mt-2 space-y-1 text-sm">
                        <li>Large websites may take longer to analyze completely</li>
                        <li>Try analyzing a specific page instead of the entire site</li>
                        <li>Check if the target website is responding slowly</li>
                        <li>Consider running the analysis during off-peak hours</li>
                        <li>The backend may still be processing - wait a moment and try again</li>
                      </ul>
                    </div>
                  )}
                </div>
